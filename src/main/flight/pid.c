...
static FAST_RAM_ZERO_INIT filterApplyFnPtr dtermNotchApplyFn = nullFilterApply;
static FAST_RAM_ZERO_INIT biquadFilter_t dtermNotch[3];
static FAST_RAM_ZERO_INIT filterApplyFnPtr dtermLowpassApplyFn = nullFilterApply;
...
// Use the FAST_CODE directive to avoid this code from being inlined into ITCM RAM. We accept the
...
float classicPids(const pidProfile_t *pidProfile, int axis, float errorRate, float dynCi, float iDT, float currentPidSetpoint) 
{
    ...
    // -----calculate I component
    float ITermNew = constrainf(pidData[axis].I + pidCoefficient[axis].Ki * errorRate * dynCi, -itermLimit, itermLimit);
    if (!mixerIsOutputSaturated(axis, errorRate) || ABS(ITermNew) < ABS(pidData[axis].I)) {
        // Only increase ITerm if output is not saturated
    ...
    // -----calculate D component
    // apply filters
    float gyroRateFiltered = dtermNotchApplyFn((filter_t *) &dtermNotch[axis], gyro.gyroADCf[axis]);
...
void pidController(const pidProfile_t *pidProfile, const rollAndPitchTrims_t *angleTrim, timeUs_t currentTimeUs)
{
    ...
    const float iDT = 1.0f/(currentTimeUs - previousTimeUs) * 0.000001f; //divide once
    // Dynamic i component,
    ...
    for (int axis = FD_ROLL; axis <= FD_YAW; axis++) {
        ...
        if ((pidProfile->crash_recovery || FLIGHT_MODE(GPS_RESCUE_MODE)) && !gyroOverflowDetected()) {
